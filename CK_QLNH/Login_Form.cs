...
namespace CK_QLNH
{
    public partial class Login_Form : Form
    {
        ...
        {
            int borderRadius = 20; // Độ cong của góc, có thể điều chỉnh tùy ý

            // Tạo một đối tượng Graphics từ sự kiện Paint
            Graphics graphics = e.Graphics;

            // Tạo một đối tượng Region với hình dạng bo góc
            Region region = new Region(
                new Rectangle(0, 0, ClientSize.Width, ClientSize.Height));
            region.MakeRoundedRect(new RectangleF(0, 0, ClientSize.Width, ClientSize.Height),
                new SizeF(borderRadius, borderRadius));

            // Thiết lập Region của Form thành hình dạng bo góc
            Region = region;
        ...
        private void buttonLogin_Click(object sender, EventArgs e)
        {
            ...
            if (radioButtonQL.Checked == true)
            {
                ...
                {
                    if (textBoxUser.Text == "quanly1")
                    {
                        MessageBox.Show("Không phải ca làm của quản lý 1!!");
                    }
                ...
                {
                    if (textBoxUser.Text == "quanly2")
                    {
                        MessageBox.Show("Không phải ca làm của quản lý 2!!");
                    }
                    else if ((table.Rows.Count > 0))
                    {
                        MainQuanLy form1 = new MainQuanLy();
                    ...
                    {
                        MessageBox.Show("Vui lòng chọn đúng vai trò", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
            if (radioButtonNV.Checked == true)
            {
                ...
                {
                    MessageBox.Show("Chọn đúng vai trò của bạn", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
    }
