...
namespace CK_QLNH
{
    public partial class QLBan_UC : UserControl
    {
        ...
        public void fillGrid(SqlCommand command)
        {
            ...
            labelSoBan.Text = "Số bàn ăn: " + dataGridViewQlBan.Rows.Count;
        }
        private void ButtonAddBanAn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (tenban.Trim() == "")
                {
                    MessageBox.Show("Thêm tên bàn", "Add Table", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else if (banan.checkTenBanAn(tenban))
                {
                    if (banan.insertBanAn(id, tenban, soluong, giaban, tinhtrang))
                    {
                        MessageBox.Show("Bàn mới đã được thêm", "Add Table", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Bàn mới chưa được thêm", "Add Table", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
                {
                    MessageBox.Show("Tên này đã tồn tại", "Add Table", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Add Table", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            fillGrid(new SqlCommand("SELECT Id as N'Mã Bàn', tenban as N'Tên Bàn', soluong as N'Số Lượng Khách', giaban as N'Giá Bàn', tinhtrang as N'Tình Trạng' FROM QLBanAn"));
            YeuCau();
        }
        ...
        private void buttonEdit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Thông tin bàn được cập nhật thành công", "Edit Table", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("Thông tin bàn chưa được cập nhật", "Edit Table", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Edit Table", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            fillGrid(new SqlCommand("SELECT Id as N'Mã Bàn', tenban as N'Tên Bàn', soluong as N'Số Lượng Khách', giaban as N'Giá Bàn', tinhtrang as N'Tình Trạng' FROM QLBanAn"));
        }
        ...
        private void ButtonRemove_Click(object sender, EventArgs e)
        {
            ...
            {
                string tenban = TextBoxTenBan.Text;
                if (MessageBox.Show("Bạn có chắc muốn xoá bàn này", "Delete Table", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (banan.deleteBanAn(tenban))
                    {
                        MessageBox.Show("Bàn xoá thành công", "Remove Table", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Bàn chưa được xoá", "Remove Table", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
            catch
            {
                MessageBox.Show("Xin hãy nhập ID hợp lệ", "Remove Table", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        ...
        private void buttonTim_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlCommand command = new SqlCommand("SELECT Id as N'Mã Bàn', tenban as N'Tên Bàn', soluong as N'Số Lượng Khách', giaban as N'Giá Bàn', tinhtrang as N'Tình Trạng' FROM QLBanAn WHERE CONCAT(tenban,soluong,giaban) LIKE '%" + textBoxSearch.Text + "%'");
                fillGrid(command);
