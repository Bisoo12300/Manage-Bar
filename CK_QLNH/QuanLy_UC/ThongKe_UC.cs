    ...
    public partial class ThongKe_UC : UserControl
    {
        ...
        private void buttonDoanhThu_Click(object sender, EventArgs e)
        {
            try
            {
                if (radioButton1Ngay.Checked == true)
                {
                    ...
                    {
                        SqlCommand command = new SqlCommand(" SELECT idban as N'Mã Bàn', tenmonhd as N'Tên Món', soluong as N'Số Lượng Món', giathanh as N'Giá Tổng', thoigian as N'Thời Gian' FROM DoanhThu WHERE(DATEPART(yy, thoigian) = '" + yy + "' AND    DATEPART(mm, thoigian) = '" + dd + "' AND    DATEPART(dd, thoigian) = '" + kk + "')", mydb.GetConnection);
                    ...
                    {
                        SqlCommand commandban = new SqlCommand(" SELECT idban as N'Mã Bàn', sotien as N'Số Tiền', date as N'Thời Gian' FROM DoanhThuBan WHERE(DATEPART(yy, date) = '" + yy + "' AND    DATEPART(mm, date) = '" + dd + "' AND    DATEPART(dd, date) = '" + kk + "')", mydb.GetConnection);
                ...
                if (radioButton1Thang.Checked == true)
                {
                    ...
                    if (radioButtonDTMon.Checked == true)
                    {
                        SqlCommand command = new SqlCommand(" SELECT idban as N'Mã Bàn', tenmonhd as N'Tên Món', soluong as N'Số Lượng Món', giathanh as N'Giá Tổng', thoigian as N'Thời Gian' FROM DoanhThu WHERE(DATEPART(yy, thoigian) = '" + yy + "' AND    DATEPART(mm, thoigian) = '" + month + "' )", mydb.GetConnection);
                        SqlDataAdapter adapter = new SqlDataAdapter(command);
                        DataTable table = new DataTable();
                        adapter.Fill(table);
                        dataGridViewDTMon.DataSource = table;
                    ...
                    if (radioButtonDTBan.Checked == true)
                    {
                        SqlCommand commandban = new SqlCommand(" SELECT idban as N'Mã Bàn', sotien as N'Số Tiền', date as N'Thời Gian' FROM DoanhThuBan WHERE(DATEPART(yy, date) = '" + yy + "' AND    DATEPART(mm, date) = '" + month + "' )", mydb.GetConnection);
                        SqlDataAdapter adapterban = new SqlDataAdapter(commandban);
                        DataTable tableban = new DataTable();
                        adapterban.Fill(tableban);
                        dataGridViewDTMon.DataSource = tableban;
                ...
                if (radioButtonNam.Checked == true)
                {
                    ...
                    if (radioButtonDTMon.Checked == true)
                    {
                        SqlCommand command = new SqlCommand(" SELECT idban as N'Mã Bàn', tenmonhd as N'Tên Món', soluong as N'Số Lượng Món', giathanh as N'Giá Tổng', thoigian as N'Thời Gian' FROM DoanhThu WHERE(DATEPART(yy, thoigian) = '" + year + "')", mydb.GetConnection);
                        SqlDataAdapter adapter = new SqlDataAdapter(command);
                        DataTable table = new DataTable();
                        adapter.Fill(table);
                        dataGridViewDTMon.DataSource = table;
                    ...
                    if (radioButtonDTBan.Checked == true)
                    {
                        SqlCommand commandban = new SqlCommand(" SELECT idban as N'Mã Bàn', sotien as N'Số Tiền', date as N'Thời Gian' FROM DoanhThuBan WHERE(DATEPART(yy, date) = '" + year + "')", mydb.GetConnection);
                        SqlDataAdapter adapterban = new SqlDataAdapter(commandban);
                        DataTable tableban = new DataTable();
                        adapterban.Fill(tableban);
                        dataGridViewDTMon.DataSource = tableban;
            ...
            {
                MessageBox.Show(ex.Message, "Doanh Thu", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void buttonMonTop_Click(object sender, EventArgs e)
        {
            ...
            if (radioButton1Ngay.Checked == true)
            {
                ...
                SqlCommand command = new SqlCommand(" SELECT sum(soluong) as N'Số Lượng', tenmonhd as N'Tên Đồ Uống' FROM DoanhThu WHERE(DATEPART(yy, thoigian) = '" + yy + "' AND    DATEPART(mm, thoigian) = '" + dd + "' AND    DATEPART(dd, thoigian) = '" + kk + "') GROUP BY tenmonhd ORDER BY N'Số Lượng' DESC", mydb.GetConnection);
                SqlDataAdapter adapter = new SqlDataAdapter(command);
                DataTable table = new DataTable();
                adapter.Fill(table);
                dataGridViewDTMon.DataSource = table;
                ...
                try
                {
                    ...
                    for (int i = 0; i < len; i++)
                    {
                        ...
                        BieuDoCot.Series["Do Uong"].Points.AddXY(s, k);
                    }
                }
            ...
            if (radioButton1Thang.Checked == true)
            {
                ...
                SqlCommand command = new SqlCommand(" SELECT sum(soluong) as N'Số Lượng', tenmonhd as N'Tên Đồ Uống' FROM DoanhThu WHERE(DATEPART(yy, thoigian) = '" + yy + "' AND    DATEPART(mm, thoigian) = '" + month + "' ) GROUP BY tenmonhd ORDER BY N'Số Lượng' DESC", mydb.GetConnection);
                SqlDataAdapter adapter = new SqlDataAdapter(command);
                DataTable table = new DataTable();
                adapter.Fill(table);
                dataGridViewDTMon.DataSource = table;
            ...
            if (radioButtonNam.Checked == true)
            {
                ...
                SqlCommand command = new SqlCommand(" SELECT sum(soluong) as N'Số Lượng', tenmonhd as N'Tên Đồ Uống' FROM DoanhThu WHERE(DATEPART(yy, thoigian) = '" + year + "') GROUP BY tenmonhd ORDER BY N'Số Lượng' DESC", mydb.GetConnection);
                SqlDataAdapter adapter = new SqlDataAdapter(command);
                DataTable table = new DataTable();
                adapter.Fill(table);
                dataGridViewDTMon.DataSource = table;
        ...
        private void buttonPrint_Click(object sender, EventArgs e)
        {
            ...
            foreach (Section section in wordDoc.Sections)
            {
                ...
                headerRange.Text = "THỐNG KÊ QUÁN BAR\n" + time;
            ...
            // Lưu thông tin 
            object filename = @"D:\ThongKeQuanBar.docx";
