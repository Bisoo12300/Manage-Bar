    ...
    public partial class Order_Form : Form
    {
        ...
        private void ButtonLuuHD_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (sl <= 0)
                {
                    MessageBox.Show("Món Này Tạm Hết Số Lượng Bán Trong Ngày. Bạn Vui Lòng Oder Món Khác");
                }
                else if (slcheck < 0)
                {
                    MessageBox.Show("Món Này Không Đủ Số Lượng Để Bạn Order. Nhà Hàng Chỉ Còn " + sl + " Phần. Bạn Có Thể Oder Số Lượng Khác Hoặc Vui Lòng Gọi Món Mới");
                }
                else
                {
                    ...
                    fillGrid(new SqlCommand("SELECT idban as N'Mã Bàn', tenmonhd as N'Tên Món', soluong as N'Số Lượng Món', giathanh as N'Giá Tổng', thoigian as N'Thời Gian' FROM HoaDon WHERE idban = '" + ban.IdBanKey.Trim() + "'"));
                }
            }
            catch (Exception ex)
            {
            ...
            SqlCommand command1 = new SqlCommand(" SELECT idban as N'Mã Bàn', tenmonhd as N'Tên Món', soluong as N'Số Lượng Món', giathanh as N'Giá Tổng', thoigian as N'Thời Gian' FROM HoaDon WHERE idban = '" + name + "'", db.GetConnection);
        ...
        private void buttonEdit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (hoadon.updateHoaDon(tenmonhd, soluong, giathanh, thoigian))
                {
                    ...
                    MessageBox.Show("Thông tin món đã được thay đổi", "Edit Menu", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("Thông tin món chưa được thay đổi", "Edit Menu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private void ButtonRemove_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Bạn có chắc muốn xoá món này ???", "Delete Menu", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (hoadon.deleteMonAn(tenm))
                    {
                        ...
                        MessageBox.Show("Món xoá thành công !!!", "Remove Menu", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Món chưa xoá được !!!", "Remove Menu", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
            catch
            {
                MessageBox.Show("Làm ơn nhập ID hợp lệ", "Remove Menu", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            fillGrid(new SqlCommand("SELECT idban AS N'Mã Bàn', tenmonhd AS N'Tên Món', soluong AS N'Số Lượng', giathanh AS N'Giá Thành', thoigian AS N'Thời Gian' FROM HoaDon WHERE idban = '" + ban.IdBanKey.Trim() + "'"));
        ...
        private void buttonHuy_Click(object sender, EventArgs e)
        {
            ...
            string idban = textBoxIdBan.Text;
            if (MessageBox.Show("Bạn có chắc muốn huỷ món này", "Delete Menu", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                MessageBox.Show("Huỷ thành công !!", "Remove Menu", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            labelTongTien.Text = "";
            fillGrid(new SqlCommand("SELECT idban AS N'Mã Bàn', tenmonhd AS N'Tên Món', soluong AS N'Số Lượng', giathanh AS N'Giá Thành', thoigian AS N'Thời Gian' FROM HoaDon WHERE idban = '" + ban.IdBanKey.Trim() + "'"));
        }
        ...
        {
            MessageBox.Show("Đã Chuyển Cho Quầy");
            Close();
        }
