...
namespace CK_QLNH
{
    public partial class Ban_Form : Form
    {
        ...
        private void ButtonLocBan_Click(object sender, EventArgs e)
        {
            ...
            else if (comboBox1.Text == "Bàn Party")
            {
                SqlCommand command = new SqlCommand(" SELECT Id as N'Mã Bàn Ăn', tenban as N'Tên Bàn', soluong as N'Số Lượng Khách', giaban as N'Giá Bàn' FROM QLBanAn WHERE soluong >= 6 AND soluong <=12", mydb.GetConnection);
                SqlDataAdapter adapter = new SqlDataAdapter(command);
                DataTable table = new DataTable();
                adapter.Fill(table);
